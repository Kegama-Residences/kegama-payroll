...
export type TabType = 'runs' | 'employees' | 'reports' | 'settings';
...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex flex-col w-64 bg-slate-950 text-slate-200 border-r border-slate-800 flex-shrink-0 h-screen sticky top-0">
        {/* Branding */}
        <div className="p-5 border-b border-slate-800/80">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-lg bg-orange-600 text-white flex items-center justify-center font-black shadow-md shadow-orange-600/25 flex-shrink-0 p-1.5">
        ...
        <div className="p-3.5">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-orange-600 hover:bg-orange-500 text-white font-bold py-2.5 px-3.5 rounded-lg shadow-md shadow-orange-600/20 transition active:scale-[0.98] text-xs"
          >
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg font-medium text-xs transition text-left ${
                  isActive
                    ? 'bg-slate-800 text-orange-400 font-bold border border-orange-500/30 shadow-xs'
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-slate-950/95 backdrop-blur-md border-t border-slate-800 px-2 pb-safe flex items-center justify-around shadow-2xl">
        {navItems.map((item) => {
          const isActive = currentTab === item.id;
          return (
            <button
              key={item.id}
              onClick={() => handleTabClick(item.id)}
              className={`flex flex-col items-center py-2 px-3 transition text-[10px] font-medium ${
                isActive
